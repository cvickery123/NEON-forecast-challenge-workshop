...
derulotest<-left_join(combined_met_df1, jasontest, by=c("datetime", "site_id", "parameter")) 
 # derulotest %>% filter(site_id=="BARC") %>% select(datetime, site_id, air_temperature, airtemp_1daylag, realdatetime, parameter) %>% view()

#NEXT STEP::::: need to write code to paste sys.date air temperature into all the airtemp_1daylag parameters for sys.date as well 
```
...
###QUESTION: NA.OMIT() HERE GIVES ME ZEROS FOR ALL OF MY SITE TARGETS, STOPPING THE MODEL. 
...
#process uncertainty
mod <- predict(fit, site_target) |> na.omit()
