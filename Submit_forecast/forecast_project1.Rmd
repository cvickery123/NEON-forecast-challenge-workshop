...
# 
# noaa_past_mean2 <-  neon4cast::noaa_stage3() |> 
...
#   mutate(air_temperature = air_temperature - 273.15)
# 
# noaa_future_mean2 <- neon4cast::noaa_stage2(start_date = noaa_forecast_start) |> 
...
# 
# combined_met_df <- bind_rows(noaa_past_mean2, noaa_future_mean2)
# 
...
# lagdf<-NULL
# 
# for(i in 1:length(combined_met_df$site_id)) {  
#   
#   curr_site <- combined_met_df$site_id[i]
#   
#   for(j in 1:length(combined_met_df$parameter)){
#     curr_param <- combined_met_df$parameter[j]
#     
#     jason<-combined_met_df %>% filter(site_id==curr_site) %>% 
#       filter(parameter==curr_param) 
#     
#     jason$datetimeplus1<-jason$datetime + days(1)
#     
#     lagdf <- dplyr::bind_rows(lagdf, jason) 
#   }
#     message(curr_site, 'forecast run')
# }
# 
# lagdf<-lagdf %>% select(c(air_temperature,datetimeplus1))
...
combined_met_df1 <- bind_rows(noaa_past_mean, noaa_future_mean)

jasontest<-combined_met_df1 %>% select(c(air_temperature, datetime))
jasontest$datetimeplus1<-jasontest$datetime + days(1)
colnames(jasontest)<-c("airtemp_1daylag", "realdatetime", "datetime")

derulotest<-left_join(combined_met_df1, jasontest, by="datetime")
```
...
for(i in 1:length(lake_sites$field_site_id)) {  
  ...
  fit <- lm(site_target$temperature ~ site_target$airtemp_1daylag + site_target$relative_humidity + site_target$windspeed)
...
sigma <- sd(residuals, na.rm = TRUE) # Process Uncertainty Noise Std Dev.; this is your sigma
  ...
  forecasted_temperature <- param_df$beta1 + param_df$beta2 * site_drivers$airtemp_1daylag + site_drivers$relative_humidity * param_df$beta3 + site_drivers$windspeed * param_df$beta4 + rnorm(n_members, mean = 0, sd = sigma)
