...
for(i in 1:length(lake_sites$field_site_id)) {  
  ...
  fit <- lm(site_target2$temperature ~ site_target2$relative_humidity + site_target2$watertemp_1daylag + site_target2$windspeed + site_target2$air_temperature)
  ...
  if (site_target2_todaylag$datetime!=Sys.Date()) {
    ...
    filter(datetime == backcast_dates[j]) %>% 
      ...
      backcast_temp <- fit$coefficients[1] + 
        fit$coefficients[2] * site_drivers_back$relative_humidity +  fit$coefficients[3] * site_drivers_back$watertemp_1daylag + fit$coefficients[4] * site_drivers_back$windspeed + fit$coefficients[5] * site_drivers_back$air_temperature
...
filledspace<-NULL
  ...
  forecasted_temperature <- param_df$beta1 + param_df$beta2 * site_drivers$relative_humidity + param_df$beta3 * site_drivers$watertemp_1daylag + param_df$beta4 * site_drivers$windspeed + param_df$beta5 * site_drivers$air_temperature + rnorm(n_members, mean = 0, sd = sigma)
...
summary_RH.WTL.WS.AT<-summary_fit
```
