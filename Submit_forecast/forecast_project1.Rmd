...
noaa_future_daily_mean<-noaa_future_daily

for(i in 1:length(lake_sites$field_site_id)){
  curr_site <- lake_sites$field_site_id[i]
  
  usher<-noaa_future_daily %>% filter(site_id==curr_site)
  
  for(p in 1:30){
    currentparameter<-usher %>% filter(parameter==p)
    currentparameter %>% summarize(airtempmean_byparameter = mean(air_temperature))
...
jasontest<-combined_met_df1 %>% select(c(air_temperature, datetime, site_id, parameter))
...
# derulotest %>% filter(site_id=="TOOK") %>% select(datetime, site_id, air_temperature, airtemp_1daylag, realdatetime, parameter) %>% view()
```
