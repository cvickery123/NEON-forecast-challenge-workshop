...
for(i in 1:length(lake_sites$field_site_id)) {  
  ...
  fit <- lm(site_target2$temperature ~ site_target2$airtemp_1daylag + site_target2$watertemp_1daylag)
  ...
  if (site_target2_todaylag$datetime!=Sys.Date()) {
    ...
    filter(datetime == backcast_dates[j]) %>% 
      ...
      backcast_temp <- fit$coefficients[1] + 
        fit$coefficients[2] * site_drivers_back$airtemp_1daylag +  fit$coefficients[3] * site_drivers_back$watertemp_1daylag 
...
filledspace<-NULL
  ...
  forecasted_temperature <- param_df$beta1 + param_df$beta2 * site_drivers$airtemp_1daylag + param_df$beta3 * site_drivers$watertemp_1daylag + rnorm(n_members, mean = 0, sd = sigma)
